...
export function createPerfectCard({
  infinitive,
  auxiliary,
  participle,
  forms,
  ...
  const draw = () => {
    ...
    ctx.fillText("PERFEKT", W / 2, 220);

    // Infinitive
    ctx.fillStyle = "rgba(20,20,20,0.9)";
    ctx.font = "600 72px Arial";
    ctx.fillText(infinitive, W / 2, 320);

    // Partizip II (past participle) — smaller, below infinitive
    ctx.fillStyle = "rgba(90,71,199,0.9)";
    ctx.font = "500 42px Arial";
    ctx.fillText(`Partizip II: ${participle}`, W / 2, 380);

    // Auxiliary hint
    ctx.fillStyle = "rgba(100,100,100,0.75)";
    ctx.font = "500 34px Arial";
    ctx.fillText(`Hilfsverb: ${auxiliary === "haben" ? "haben" : "sein"}`, W / 2, 430);
    ...
    ctx.lineWidth = 6;
    ctx.beginPath();
    ctx.moveTo(W / 2 - 220, 470);
    ctx.lineTo(W / 2 + 220, 470);
    ctx.stroke();

    // Conjugation rows (Perfekt forms)
    ctx.textAlign = "left";
